...
import { dataHubApi, SACUploadResponse } from '../requests/dataHub';
...
export function SACUploadCard({ lastUpdated, recordCount, onUploadSuccess }: SACUploadCardProps) {
  ...
  const handleUpload = async (fileToUpload: File) => {
    ...
    try {
      const uploadResult = await dataHubApi.uploadSAC(fileToUpload);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      {uploadState === 'success' && result && (
        <>
          <UploadResultPanel type="sac" result={result} onClose={handleReset} />
          <button
            onClick={handleReset}
