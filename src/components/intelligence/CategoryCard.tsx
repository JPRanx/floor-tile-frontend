    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">
              {category.product_count} {t('intelligence.categoryCard.products', 'productos')}
            </p>
          </div>
        </div>
        <TrendArrow
      ...
      <div className="mb-4">
        <div className="flex items-end justify-between mb-2">
          <p className="text-slate-500 text-xs uppercase tracking-wide">{t('intelligence.categoryCard.warehousePct', '% Bodega')}</p>
        ...
        <p className="text-slate-500 text-xs mt-1">
          {t('intelligence.categoryCard.inWarehouse', '{{m2}} en bodega', { m2: formatM2(category.warehouse_m2) })}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          <p className="text-slate-500 text-xs uppercase tracking-wide mb-1">{t('intelligence.categoryCard.velocity', 'Velocidad')}</p>
          <p className="text-emerald-400 font-bold text-lg">
            {category.avg_velocity_m2_day.toFixed(1)}
          </p>
          <p className="text-slate-500 text-xs">{t('intelligence.categoryCard.m2DayAvg', 'm²/día prom')}</p>
        </div>
        <div>
          <p className="text-slate-500 text-xs uppercase tracking-wide mb-1">{t('intelligence.categoryCard.coverage', 'Cobertura')}</p>
          {category.avg_warehouse_days !== null ? (
            <>
              ...
              <p className="text-slate-500 text-xs">{t('intelligence.categoryCard.daysAvg', 'días prom')}</p>
            </>
          ) : (
            <p className="text-slate-500 text-sm">{t('intelligence.categoryCard.noData', 'Sin datos')}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-slate-700/50">
        {hasRisk ? (
          <span className="px-2 py-1 rounded-full bg-rose-500/20 text-rose-300 text-xs font-medium">
            {t('intelligence.categoryCard.atRisk', '{{count}} en riesgo', { count: category.products_at_risk })}
          </span>
        ) : (
          <span className="px-2 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-medium">
            {t('intelligence.categoryCard.healthyStock', 'Stock saludable')}
          </span>
        )}
        {lowCoverage && (
          <span className="px-2 py-1 rounded-full bg-amber-500/20 text-amber-300 text-xs font-medium">
            {t('intelligence.categoryCard.lowCoverage', 'Baja cobertura')}
          </span>
        )}
      </div>
    </div>
