...
import type { CommittedOrderPreview, CommittedOrderResponse, CommittedOrderModification } from '../requests/dataHub';
...
const columns: EditableColumn[] = [
  { key: 'sku', label: 'Producto', editable: false, type: 'text' as const, width: 'w-48' },
  { key: 'quantity_committed', label: 'Cant. Comprometida', editable: true, type: 'number' as const, width: 'w-36' },
  { key: 'current_stock', label: 'Existencia', editable: false, type: 'number' as const, width: 'w-28' },
  { key: 'available_qty', label: 'Disponible', editable: false, type: 'number' as const, width: 'w-28' },
  { key: 'order_reference', label: 'Pedido', editable: false, type: 'text' as const, width: 'w-24' },
];

export function CommittedOrdersCard({ onUploadSuccess }: Props) {
  ...
  const handleUpload = async (fileToUpload: File) => {
    ...
    try {
      const previewData = await dataHubApi.previewCommittedOrders(fileToUpload);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const modsArray: CommittedOrderModification[] = Array.from(modifications.entries()).map(
        ([sku, changes]) => ({
          sku,
          ...(changes.quantity_committed !== undefined ? { quantity_committed: changes.quantity_committed as number } : {}),
        })
      );
      const deletionsArray = Array.from(deletions);

      const uploadResult = await dataHubApi.confirmCommittedOrders(
        preview.preview_id,
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-semibold text-gray-900">Pedidos Comprometidos</h3>
          <p className="text-sm text-gray-500">Compromisos de clientes desde SIESA</p>
        </div>
        <svg
            ...
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex-1">
                ...
                <div className="mt-2 space-y-1 text-sm text-green-700">
                  ...
                  <p>Fecha corte: {formatDateForDisplay(result.snapshot_date)}</p>
                  <p>{result.message}</p>
                </div>
              </div>
            </div>
